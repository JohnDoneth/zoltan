...
pub const Lua = struct {
    ...
    pub fn Function(comptime T: type) type {
        ...
        //         comptime const retValCount = switch (@typeInfo(RetType.?)) {
        //             .Void => 0,
    ...
    //         switch (@typeInfo(T)) {
    //             .Bool => {
    //                 const res = lualib.lua_toboolean(L, -1);
    ...
    //             },
    //             .Struct => |_| {
    //                 comptime const funIdx = std.mem.indexOf(u8, @typeName(T), "Function") orelse -1;
    //                 comptime const tblIdx = std.mem.indexOf(u8, @typeName(T), "Table") orelse -1;
    //                 comptime const refIdx = std.mem.indexOf(u8, @typeName(T), "Ref(") orelse -1;
    //                 if (funIdx >= 0 or tblIdx >= 0 or refIdx >= 0) {
    //                     if (deallocate) {
    //                         value.?.destroy();
    //                     }
